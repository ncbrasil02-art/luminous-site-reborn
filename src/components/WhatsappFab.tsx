...
export function WhatsappFab() {
  ...
  // Intensifica o pulso quando o foguete está pousando (entre 35% e 50% do scroll)
  const pulseIntensity = useTransform(scrollYProgress, [0.3, 0.45], [1, 1.5]);
  ...
  return (
    <motion.a
      ...
      className="fixed bottom-6 right-6 z-40 grid h-14 w-14 place-items-center rounded-none bg-gradient-primary text-primary-foreground glow-md transition-transform hover:scale-110"
      animate={{
        boxShadow: [
    ...
    >
      <motion.span 
        ...
        className="absolute inset-0 rounded-none bg-gradient-primary opacity-60 blur-xl animate-glow-pulse" 
      />
      
      {/* Luz suave de atenção que sincroniza com o pouso */}
      <motion.div
        style={{
          opacity: useTransform(scrollYProgress, [0.35, 0.45, 0.55], [0, 1, 0.3])
        }}
