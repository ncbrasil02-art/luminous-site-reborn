...
def get_image_files():
    ...
    for f in NEWS_IMAGES_DIR.iterdir():
        if f.is_file() and f.suffix.lower() in ['.jpg', '.jpeg', '.png', '.webp']:
            ...
            # Remove all digits and hyphens for a very fuzzy match
            simplified = re.sub(r'[^a-z]', '', base)
            if simplified and len(simplified) > 5:
                if simplified not in images:
                    images[simplified] = f.name
    return images
...
def repair_news_data():
    ...
    for post in data:
        ...
        # Only try to repair if it's default or seems wrong
        if current_img != '/news/default-nc.jpg' and not current_img.startswith('/news/download'):
             # We might still want to try to find a BETTER match if the current one is 'download-X'
             if not current_img.startswith('/news/download'):
                 continue
        ...
        # Strategy 2: Fuzzy simplified slug match
        if not found_image:
            simplified_slug = re.sub(r'[^a-z]', '', slug)
            if simplified_slug in image_files:
                found_image = image_files[simplified_slug]

        # Strategy 3: thumb_id match
        ...
        if not found_image and 'content' in post:
            ...
            for img_path in content_imgs:
                ...
                if img_name in image_files and 'default-nc' not in img_name:
                    found_image = image_files[img_name]
                    break
                elif clean_img_name in image_files and 'default-nc' not in clean_img_name:
